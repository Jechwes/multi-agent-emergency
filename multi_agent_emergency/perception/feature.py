...
class FeatureExt():
    ...
    def update(self):
        ...
        print("location:", [self.current_loc.x, self.current_loc.y])
        ...
        self.all_wp = self.branch_wp(self.cur_lane)
        self.wp_list = self.wp_list_extract(self.cur_lane)
        self.lane_width = self.cur_lane.lane_width

        if self.is_junction:
            jun = self.cur_lane.get_junction()
            waypoints = jun.get_waypoints(carla.LaneType.Any)
            for waypoint in waypoints:
                for wp in waypoint:
                    if wp.road_id == self.cur_lane.road_id:
                        self.wp_list = self.wp_list_extract(wp)
    ...
    def branch_wp(self, wp, max_distance=80):
        seq = 1.0
        ap_l = [wp.next(0.01)[0]]
        while True:
    ...
    def point_display(self, my_color):
        ...
        for wp in self.all_wp:
            self.world.debug.draw_point(wp.transform.location, size=0.1, color=my_color, life_time=self.show_dt)
