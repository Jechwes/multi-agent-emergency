...
class SafetyFilter:
    ...
    def stage_risk_cost(
        ...
        # Mirror the ped_is_threat logic from evaluate(): score proximity cost
        # for any pedestrian on a drivable lane OR moving toward one, not just
        # same-lane pedestrians.
        ped_is_threat = False
        if ped_on_lane is not None:
            if ped_on_lane in self.drivable_lanes:
                ped_is_threat = True
